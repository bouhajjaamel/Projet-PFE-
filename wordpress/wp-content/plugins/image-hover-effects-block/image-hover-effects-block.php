...
function iheg_render_callback($attributes){
    ...
    if($font_handle && $font_handle != 'inherit'){
        ...
        wp_enqueue_style( 'iheg-google-font-' . $font_handle, 'https://fonts.googleapis.com/css2?family=' . $font_family . ':wght@' . $attributes['title_font_weight'] . '&display=swap', array(), false );
    }
    ...
    if($font_handle && $font_handle != 'inherit'){
        $font_family = esc_attr( str_replace( ' ', '+', $font_handle ));
        $font_handle = str_replace( ' ', '-', strtolower( $font_handle ));
        wp_enqueue_style( 'iheg-google-font-' . $font_handle, 'https://fonts.googleapis.com/css2?family=' . $font_family . ':wght@' . $attributes['desc_font_weight'] . '&display=swap', array(), false );
    }
    ...
    // Render Output...
    $iheg_block = "";
    $iheg_block .= ($attributes['linktourl'] != '') ? '<a class="iheg-wrapper" href="'. $attributes['linktourl'] .'" '. (($attributes['addtarget'] == true)? 'target="_blank"': '') .' '. (($attributes['addrel'] == true) ? 'rel="nofollow"':'') .'>' : '<div class="iheg-wrapper">';
